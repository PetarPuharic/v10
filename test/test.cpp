	...
	public:
		...
		TEST_METHOD(test_02a)
		{
			...
			*/
			...
			//Iota ima generator koji automatski povećava vrijednost
		...
		TEST_METHOD(test_02b)
		{
			// generate
			std::vector<int> v(10);
			std::generate(v.begin(), v.end(), [n = 1]() mutable { int val = n; n += 2; return val; });
			Assert::IsTrue(std::is_sorted(v.cbegin(), v.cend()));
		...
		TEST_METHOD(test_05c)
		{
			...
			Assert::AreEqual(2ll, number_in_first_quadrant); //Top Right Quadrant
		}
		...
		TEST_METHOD(test_12)
		{
			...
			std::adjacent_difference(atp_points.begin(), atp_points.end(), diffs.begin());
			auto smallest_difference = std::min_element(diffs.begin() + 1, diffs.end());
